...
apiController.getOptions = (req, res, next) => {
  ...
  const randomYear =
		2015 + Math.floor(Math.random() * (today.getFullYear() - 2015));
  ...
  fetch(
    `https://api.nasa.gov/planetary/apod?start_date=${start_date}&end_date=${end_date}&api_key=${apiKey}`
  )
    .then((res) => res.json())
  //send back the image url for source and the title for the answer
    .then((data) => {
      ...
      const secondRandomIndex =
				firstRandomIndex + Math.ceil(Math.random() * 10);
      const thirdRandomIndex =
				secondRandomIndex + Math.ceil(Math.random() * 10);
